...
def run_gp_strategy(train_df: pd.DataFrame, test_df: pd.DataFrame, ticker: str):
    ...
    try:
        ...
        # 2. 训练符号回归模型 - 使用平衡的参数（避免退化但不过度训练）
        print("   Training GP model...")
        gp_model = SymbolicRegressor(
            population_size=300,  # 适度增加种群大小
            generations=10,       # 适度增加进化代数
        ...
        # 🔧 新增：保存GP表达式（用于Pareto Frontier分析）
        gp_expression = str(gp_model._program)
        metrics['gp_expression'] = gp_expression
        ...
        print(f"   GP Formula: {gp_expression[:100]}...")
