...
# 策略配置
# 注意：EATA是主实验方法，baseline对比实验中不运行EATA
STRATEGY_CONFIGS = {
    'buy_and_hold': {
        'module': 'buy_and_hold',
...
class BaselineRunner:
    ...
    def run_all_strategies(self, df: pd.DataFrame, ticker: str = 'TEST',
                          train_ratio: float = 0.7,  # 修复为70%训练，30%测试，与消融实验一致
    ...
    def save_results(self, results: Dict, ticker: str, output_dir: str = None, 
        ...
        # 默认使用项目根目录的results/comparison_study/baseline_100stocks/
        if output_dir is None:
            project_root = Path(__file__).parent.parent.parent.parent
            output_dir = str(project_root / "results" / "comparison_study" / "baseline_100stocks" / "raw_results")
...
def run_real_data_experiment(ticker: str, selected_strategies=None, params=None, run_id=1):
    ...
    # 使用所有可用数据
    print(f" 使用数据: {len(df)} 条记录，时间范围: {df['date'].min()} 到 {df['date'].max()}")
