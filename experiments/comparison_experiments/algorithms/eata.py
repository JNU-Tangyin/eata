...
# __file__ = .../experiments/comparison_experiments/algorithms/eata.py
# 需要回退3层到项目根目录
...
def run_eata_strategy(train_df: pd.DataFrame, test_df: pd.DataFrame, ticker: str):
    ...
    try:
        ...
        # 打印特征信息
        print(f"   EATA使用特征数: {len(combined_df.columns)} 列")
        print(f"   包含技术指标: {len(combined_df.columns) - 7} 个")
        ...
        # 🔧 新增：保留表达式数据（用于Pareto Frontier分析）
        if 'Discovered Expressions' in core_metrics:
            performance_metrics['Discovered Expressions'] = core_metrics['Discovered Expressions']
        if 'Expression Complexities' in core_metrics:
            performance_metrics['Expression Complexities'] = core_metrics['Expression Complexities']

        equity_curve = portfolio_df.reset_index().rename(
            columns={'index': 'date', 'value': 'portfolio_value'}
        )

        # 🔧 新增：保存portfolio_df到CSV（用于生成Figure 5和Figure 6）
        from pathlib import Path
        portfolio_output_dir = Path('results/comparison_study/portfolio_data')
        portfolio_output_dir.mkdir(parents=True, exist_ok=True)
        portfolio_file = portfolio_output_dir / f'{ticker}_eata_portfolio.csv'
