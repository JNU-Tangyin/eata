...
from configs.ablation_config import ABLATION_CONFIGS, EXPERIMENT_SETTINGS, CSV_CONFIG
...
class AblationStudyRunner:
    ...
    def generate_csv_summary(self):
        ...
        特殊处理：EATA-Simple会返回4个目标函数的详细结果+平均值
                ...
                if 'detailed_results' in result:
                    ...
                    for detail in result['detailed_results']:
                        summary_data.append({
                            ...
                            'annual_return': detail.get('annual_return', ''),
                            ...
                            'rl_reward': detail.get('rl_reward', ''),
                            'experiment_id': self.experiment_id,
                    ...
                    summary_data.append({
                        ...
                        'objective': result.get('objective', 'AVERAGE'),
        ...
        # 计算Simple方法的平均结果并添加到CSV

        simple_variants = ['EATA-MSE', 'EATA-KL', 'EATA-JS', 'EATA-CVaR']

        simple_results = summary_df[summary_df['variant'].isin(simple_variants)]
        ...
        if len(simple_results) > 0:
            ...
            avg_row = {
                ...
                'variant': 'EATA-Simple-Average',

                'ticker': simple_results['ticker'].iloc[0] if len(simple_results) > 0 else 'Average',

                'annual_return': simple_results['annual_return'].mean(),

                'sharpe_ratio': simple_results['sharpe_ratio'].mean(),

                'max_drawdown': simple_results['max_drawdown'].mean(),

                'win_rate': simple_results['win_rate'].mean(),

                'volatility': simple_results['volatility'].mean(),

                'rl_reward': simple_results['rl_reward'].mean(),
            ...
            print(f"   ✅ 已添加Simple方法平均结果行（基于 {len(simple_results)} 个目标函数）")
