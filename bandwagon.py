...
class Bandwagon():
    def __init__(self, df: pd.DataFrame):
        ...
        def __prepare__(s:pd.Series)-> pd.DataFrame:
            # 获取所有股票当天的数据，这样其他函数只需要做计算即可
            d1 = [BaostockDataWorker().get_data(c) for c in s] # a list of df
            d2 = [Preprocessor(s).bundle_process() for s in d1] # 对每个df做预处理
            return d2
        
        # 准备好stocks, sectors, markets的数据
        self.stocks = __prepare__(self.stock_list.code)
        self.sectors = __prepare__(self.stock_list.sector)
        markets_code = self.stock_list.code.apply(self.market) # 对每个股票获得其大盘指数代码
        markets_code = markets_code.drop_dupulicates() # 去重，对sz50来说，就剩1个"sh.000001"     
    ...
    def market(self, ticker:str)->str:
    ...
    def stock_strength(self): # v1.1
        ...
        self.stock_list['stock_strength'] = self.stocks.apply(criteria, axis=1) # df添加一列
    ...
    def sector_strength(self, sector): # v1.2
        ...
        self.stock_list['sector_strength'] = self.sectors.apply(criteria, axis=1) # df增加一列
    ...
    def market_strength(self): # v1.2
        ...
        # 计算每个market的strength 
        self.markets['market_strength'] = self.markets.apply(criteria, axis=1)
        # 为stocks增加market字段，填入指数代码
        self.stock_list['market'] = self.market_of(self.stocks.code) # 
        # 根据指数代码market对应大盘的指数代码code，进行连接
        self.stock_list.merge(self.markets, left_on="market", right_on="code", how="left")  # df增加一列
    ...
    def stock_momentum(self, ticker): # v1.2
        ...
        self.stock_list['momentum'] = sig21(self.stocks.close.diff(1)) # df增加一列
        return self.stock_list.momentum
