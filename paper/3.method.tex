...
\textit{Financial Operators (Fixed-Window):}
\begin{align}
\mathit{A} &\rightarrow \text{delay}(\mathit{A}, 1) \mid \text{delay}(\mathit{A}, 5) \mid \text{ma}(\mathit{A}, 5) \mid \text{ma}(\mathit{A}, 10) \mid \text{ma}(\mathit{A}, 20) \mid \text{diff}(\mathit{A}, 1) \mid \text{diff}(\mathit{A}, 5) \nonumber \\
&\quad \mid \text{mom}(\mathit{A}, 10) \mid \text{max\_n}(\mathit{A}, 5) \mid \text{min\_n}(\mathit{A}, 5) \mid \text{rsi}(\mathit{A}, 14) \mid \text{vol}(\mathit{A}, 20)
\end{align}
...
\textbf{Design Rationale.} Fixed-window operators (e.g., $\text{ma}(\cdot, 10)$) rather than parameterized variants reduce the search space while preserving expressiveness. Predefined windows ($5, 10, 14, 20$) align with standard technical analysis horizons. The conditional operator $\text{ite}$ enables piecewise expressions for regime-switching strategies.
...
\begin{equation}
\text{UCB}(\mathit{s}, \mathit{a}) = \frac{Q(\mathit{s}, \mathit{a})}{N(\mathit{s}, \mathit{a})} + C \sqrt{\frac{\ln N(\mathit{s})}{N(\mathit{s}, \mathit{a})}}
\label{eq:ucb}
\end{equation}
...
To explicitly optimize for trading profitability, we introduce a Reinforcement Learning (RL) reward signal $r^{\text{rl}}$. Unlike standard regression targets that penalize point-wise errors (e.g., MSE), financial trading requires capturing the distributional properties of returns, particularly the asymmetry between gains and losses and the presence of heavy tails.

Given predictions $\hat{\mathbf{y}}^{(1)}, \ldots, \hat{\mathbf{y}}^{(k)}$ from the top-$k$ discovered expressions, we construct a prediction distribution $\mathcal{D}^{\text{pred}}$. We compare this against the distribution of actual future returns $\mathcal{D}^{\text{actual}}$ using the 1-Wasserstein distance (Earth Mover's Distance):
\begin{equation}
