...
\label{alg:pvnet}
\begin{algorithmic}[1]
\Require Derivation tree $\tau$ (or rule sequence $\boldsymbol{\sigma}$), time series $\mathbf{X} \in \mathbb{R}^{T \times d}$
\Ensure Policy $\boldsymbol{\pi}$, value $V$, profit $P$
\State $\mathbf{h}^{\text{tree}} \leftarrow \textsc{TreeLSTM}(\tau)$ \Comment{Structure-aware encoder}
\State $\mathbf{h}^{\text{seq}} \leftarrow \textsc{LSTM}_{\text{seq}}(\mathbf{X})$ \Comment{Sequence encoder}
\State $\mathbf{h} \leftarrow [\mathbf{h}^{\text{tree}}; \mathbf{h}^{\text{seq}}]$ \Comment{Concatenate}
...
\textbf{Design Rationale.} We employ a \textbf{Tree-LSTM} encoder~\cite{tai2015improved} to capture the hierarchical structure of the symbolic expression, rather than treating it as a flat sequence. This allows the network to better generalize to nested mathematical compositions. The market dynamics are encoded via a standard LSTM. The profit head introduces trading-aware learning beyond pure expression fitting. Both value and profit heads use MSE loss against respective rewards.
