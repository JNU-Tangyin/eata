...
We additionally plan a full-universe evaluation on all eligible S\&P 500 constituents under a unified backtesting protocol. Table~\ref{tab:sp500_full_placeholder} and Figure~\ref{fig:sp500_full_placeholder} provide placeholders for the planned large-scale results.

\textbf{Scope note.} To avoid overstating the current empirical scope, we separate \emph{core results} (reported on representative S\&P 500 constituents) from \emph{planned full-universe} evaluation. The placeholder artifacts are automatically generated to preserve a fully executable paper pipeline and will be replaced by real full-universe results in the final version.

\begin{table}[htbp]
\centering
\caption{Planned Full S\&P 500 Evaluation (Placeholder).}
\label{tab:sp500_full_placeholder}
...
\caption{Planned Full S\&P 500 Evaluation (Placeholder).}
\label{fig:sp500_full_placeholder}
\end{figure}
...
Table~\ref{tab:main_results} reports performance metrics averaged across our evaluation universe. EATA achieves the highest risk-adjusted returns with a \textbf{Sharpe Ratio of 0.83}, outperforming the best deep learning baseline (LightGBM, Sharpe 0.52) and the traditional Buy \& Hold strategy (Sharpe 0.42).
...
Table~\ref{tab:component_ablation} extends the analysis to other key components: MCTS search and Neural Network guidance.

\input{tables/ablation_component.tex}

\textbf{Finding (Policy Robustness vs. Search Overfitting).} The \textbf{NoMCTS} variant achieves the highest performance (SR 0.89), outperforming the full system (SR 0.55). This indicates that the neural policy has successfully learned robust, generalizable trading patterns that transfer well to unseen data. Conversely, the intensive search in \textbf{EATA (Full)} appears to ``over-optimize'' to the specific training window, fitting noise rather than signal. This highlights a critical insight: in stochastic environments like finance, \emph{less search} can sometimes yield \emph{better generalization} by preventing the discovery of spurious complexity. The neural network acts as a regularizer, distilling the essential strategy logic, whereas the full MCTS search exploits transient inefficiencies that do not persist out-of-sample.
