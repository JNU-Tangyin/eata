...
def main():
    ...
    for round_num in range(args.round + 1):
        ...
        for iter, (data, _, _, _) in enumerate(train_loader):
            ...
            best_exp, all_times, test_data, loss, mae, mse, corr, policy, reward = engine.simulate(train_data)
        ...
        if args.recording:
            ...
            # 采集 tracker 日志
            avg_loss = train_loss / train_n_samples if train_n_samples > 0 else None
            avg_mae = sum(test_maes) / len(test_maes) if len(test_maes) > 0 else None
            avg_mse = sum(test_mses) / len(test_mses) if len(test_mses) > 0 else None
            avg_corr = sum(test_corrs) / len(test_corrs) if len(test_corrs) > 0 else None
            # best_score 可用 -avg_mae 或其它你认为代表最优的指标
            best_score = -avg_mae if avg_mae is not None else None
            engine.tracker.update(
                step=round_num,
                alpha=None,
                policy=policy_list,
                value=avg_loss,
