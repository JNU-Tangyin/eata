...
def score_with_est(eq, tree_size, data, t_limit=1.0, eta=0.999):
    ...
    # 修复广播错误：确保f_pred和f_true形状兼容
    try:
        # 从全局变量获取f_true
        f_true_global = globals().get('f_true', None)
        if f_true_global is None:
            return 0.0, eq
            
        f_pred = np.asarray(f_pred)
        f_true_array = np.asarray(f_true_global)
        
        # 如果f_pred是空数组或形状不匹配，返回0分
        if f_pred.size == 0 or f_true_array.size == 0:
            return 0.0, eq
        
        # 确保形状匹配
        if f_pred.shape != f_true_array.shape:
            if f_pred.size == 1:
                f_pred = np.full_like(f_true_array, f_pred.item())
            else:
                min_len = min(len(f_pred), len(f_true_array))
                f_pred = f_pred[:min_len]
                f_true_array = f_true_array[:min_len]
        
        # 计算评分
        norm_diff = np.linalg.norm(f_pred - f_true_array, 2)
        r = float(eta ** tree_size / (1.0 + norm_diff ** 2 / f_true_array.shape[0]))
