...
class Engine(object):
    ...
    def simulate(self, data, previous_best_tree=None, alpha=None, variant_exploration_rate=None):
        ...
        if len(self.model.data_buffer) >= self.args.train_size and self.global_train_step % 10 == 0:
            self.train()  # train方法内部会检查NoNN变体并跳过
    ...
    def store_experiences(self, experiences):
        ...
        if len(self.model.data_buffer) >= self.args.train_size:
            ...
            self.train(variant_profit_loss_weight=variant_profit_loss_weight)  # train方法内部会检查NoNN变体并跳过
        else:
    ...
    def train(self, variant_profit_loss_weight=None):
        ...
        # 🔧 检查训练冻结：如果在回测阶段，则跳过训练（离线学习模式）
        if hasattr(self.model, '_freeze_training') and self.model._freeze_training:
            print("🔒 [离线学习模式] 训练已冻结，跳过训练")
            return 0
        
        # 🔧 检查NoNN变体：如果禁用神经网络，则跳过训练
        if hasattr(self.model, '_variant_skip_nn') and self.model._variant_skip_nn:
